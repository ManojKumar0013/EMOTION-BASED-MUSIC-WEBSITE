...
function applyMoodBackground(mood) {
  ...
  document.getElementById('rainLayer').classList.remove('on');
  document.getElementById('fireLayer').classList.remove('on');
  document.getElementById('breatheLayer').classList.remove('on');
  document.getElementById('heartLayer').classList.remove('on');


  if (mood === 'sad')   buildRain();
  ...
  requestAnimationFrame(() => {
    requestAnimationFrame(() => {
      if (mood === 'sad')   document.getElementById('rainLayer').classList.add('on');
      if (mood === 'angry') document.getElementById('fireLayer').classList.add('on');
      if (mood === 'calm')  document.getElementById('breatheLayer').classList.add('on');
      if (mood === 'love')  document.getElementById('heartLayer').classList.add('on');
    });
  });
...
function buildFire() {
  ...
  const fireColors = [
    ...
    'radial-gradient(circle, #ffaa00, #ff6000)',
    'radial-gradient(circle, #fff, #ffcc00)',
  ];
  for (let i = 0; i < 70; i++) {
...
function mountVuePlaylistBuilder() {
  if (vueApp) return;  

  const JAMENDO_ID = '69a97c71';
  const EMO_TAG_MAP = {
    joy:'happy', sad:'sad', angry:'energetic', fear:'ambient',
    calm:'relaxing', excited:'dance', disgust:'grunge', love:'romantic'
  };
  const ENERGY_LABELS = { 1:'Mellow', 2:'Easy', 3:'Balanced', 4:'Pumped', 5:'Maximum' };

  vueApp = Vue.createApp({
    data() {
      return {
        moods: [
          { key:'joy',     label:'Joy',     emoji:'😄', color:'#FFD700' },
          { key:'sad',     label:'Sad',     emoji:'😢', color:'#6495ED' },
          { key:'angry',   label:'Angry',   emoji:'😤', color:'#FF4500' },
          { key:'fear',    label:'Anxious', emoji:'😨', color:'#9B59B6' },
          { key:'calm',    label:'Calm',    emoji:'😌', color:'#00CED1' },
          { key:'excited', label:'Excited', emoji:'🤩', color:'#FF69B4' },
          { key:'disgust', label:'Disgust', emoji:'🤢', color:'#32CD32' },
          { key:'love',    label:'Love',    emoji:'🥰', color:'#FF1493' },
        ],
        ...
        playlist      : [],
        currentIdx    : -1,
        savedPlaylists: JSON.parse(localStorage.getItem('vpb_saved') || '[]'),
        favIds        : new Set(JSON.parse(localStorage.getItem('mt_favs') || '[]')),
      };
    },

    computed: {
      moodLabel()  { return this.moods.find(m => m.key === this.selectedMood)?.label || ''; },
      energyLabel(){ return ENERGY_LABELS[this.energy] || 'Balanced'; },
      finalName()  { return this.playlistName.trim() || `My ${this.moodLabel} Mix`; },
    },
    ...
    methods: {
      ...
      async buildPlaylist() {
        ...
        const tag    = EMO_TAG_MAP[this.selectedMood] || 'happy';
        const limit  = Math.min(this.songCount, 30);
        const offset = Math.floor(Math.random() * 40); 

        try {
          const url  = `https://api.jamendo.com/v3.0/tracks/?client_id=${JAMENDO_ID}&format=jsonpretty&limit=${limit}&offset=${offset}&tags=${tag}&include=musicinfo&audioformat=mp32`;
          const res  = await fetch(url);
          const data = await res.json();

          this.playlist = (data.results || []).map(t => ({
            id    : t.id,
            name  : t.name,
            artist: t.artist_name,
            image : t.image,
          ...
          if (!this.playlist.length) {
            showToast('⚠️ No songs found. Try a different mood.');
          } else {
            showToast(`✅ ${this.playlist.length} songs loaded!`);
          }
        } catch(e) {
          showToast('⚠️ Could not fetch songs. Check internet connection.');
        } finally {
          this.building = false;
        }
      },
      ...
      playSong(idx) {
        ...
        if (!song || !song.audio) { showToast('⚠️ No audio stream for this song.'); return; }
        this.currentIdx = idx;

        window.songQueue    = this.playlist;
        window.currentIndex = idx;
        if (typeof playSongAt === 'function') playSongAt(idx);
        showToast(`▶ Now playing: ${song.name}`);
      },

      isFav(id) { return this.favIds.has(String(id)); },
      ...
      toggleFavVue(song) {
        ...
        localStorage.setItem('mt_favs', JSON.stringify([...this.favIds]));
        if (typeof favSet !== 'undefined') {
          if (this.favIds.has(id)) favSet.add(id); else favSet.delete(id);
        }

        this.favIds = new Set(this.favIds);
      },

      removeSong(idx) {
        this.playlist.splice(idx, 1);
        if (this.currentIdx >= this.playlist.length) this.currentIdx = -1;
      },
      ...
      savePlaylist() {
        if (!this.playlist.length) { showToast('⚠️ No songs to save.'); return; }
        const entry = {
          name  : this.finalName,
          mood  : this.moodLabel,
          songs : this.playlist.length,
          data  : this.playlist,
          saved : new Date().toLocaleString(),
        };
        this.savedPlaylists.unshift(entry);
        if (this.savedPlaylists.length > 10) this.savedPlaylists.pop(); // keep max 10
        localStorage.setItem('vpb_saved', JSON.stringify(this.savedPlaylists));
      ...
      loadSaved(i) {
        ...
        document.querySelector('.vpb-song-list')?.scrollIntoView({ behavior:'smooth' });
      },

      deleteSaved(i) {
        const name = this.savedPlaylists[i].name;
  ...
  }).mount('#vuePlaylistApp');
}


document.addEventListener('DOMContentLoaded', () => {
  const _realNavTo = navTo;
  window.navTo = function(page) {
    _realNavTo(page);
    if (page === 'playlist-builder') {
      setTimeout(mountVuePlaylistBuilder, 50);
    }
  };
});
